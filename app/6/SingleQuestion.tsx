...
export const SingleQuestion: FC<SingleQuestionProps> = ({ question, index, nextQuestion }) => {
  ...
  const countAddsRef = useRef(0);

  useEffect(() => {
    const isImage = question.media?.endsWith(".png");

    if (index !== 0 && isImage && countAddsRef.current < 1) {
      setShowAdd(true);
      return;
    }
  }, [index]);
  ...
  useEffect(() => {
    setAddContent(() => introductionContent[Math.floor(Math.random() * introductionContent.length)]);
  }, [index]);
  ...
  return (
    <>
      ...
      {showAdd && (
        <div className="flex fixed h-full top-0 left-0 justify-center content-center flex-wrap p-10">
          <div className="hidden">
            <Mp3
              text={addContent}
              onEndedCallback={() => {
                setShowAdd(false);
                countAddsRef.current++;
              }}
          ...
          <p className="bg-black text-white p-5 ">{addContent}</p>
        </div>
      )}
          ...
          {showAnswer && (
            <p className="pb-2 hidden">
              <Mp3
                ...
                onEndedCallback={hendleNextQuestionWithDelay}
                onErrorCallback={hendleNextQuestionWithDelay}
                autoPlayWithdelay={0}
              />
